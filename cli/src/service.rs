...
use adavigator_runtime::RuntimeApi;
use executor::ExecutorDispatch;
use adavigator_primitives::Block;
	...
	let (rpc_builder, rpc_setup) = {
		...
		(
			move |deny_unsafe, subscription_executor: sc_rpc::SubscriptionTaskExecutor| {
				...
				rpc::create_full(deps, subscription_executor, overrides.clone())
					.map_err(Into::into)
			},
			rpc_setup,
		)
...
mod tests {
	...
	use adavigator_primitives::{Block, DigestItem};
	use codec::Encode;
	use sc_cli::SubstrateCli;
	use sc_client_api::BlockBackend;
